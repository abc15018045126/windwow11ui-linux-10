...
const getFileIconName = (filename: string): string => {
    ...
    if (filename.endsWith('.shortcut')) return 'fileGeneric'; // Placeholder
    return 'fileGeneric';
};
...
const FileExplorerApp: React.FC<AppComponentProps> = ({ setTitle, initialData }) => {
    ...
    const navigateTo = useCallback((path: string) => {
        if (path === currentPath) {
            fetchItems(); // Refresh if navigating to the same path
            return;
        };
    ...
    const shortcuts = [
        ...
        { name: 'Desktop', path: '/Desktop', icon: 'desktop' }, // Assuming desktop icon exists
    ...
    return (
        <div
            className="flex h-full bg-zinc-900 text-white select-none flex-col"
            data-context-menu-type="file-explorer-background"
            data-context-menu-payload={JSON.stringify({ path: currentPath })}
        >
            {/* Toolbar */}
                            ...
                            <div className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-10 lg:grid-cols-12 gap-4">
                                {items.map(item => (
                                    <div
                                        ...
                                        className="flex flex-col items-center p-2 rounded hover:bg-white/10 text-center"
                                        data-context-menu-type="file-explorer-item"
                                        data-context-menu-payload={JSON.stringify(item)}
                                    >
                                        <Icon iconName={getFileIconName(item.name)} className="w-12 h-12" />
