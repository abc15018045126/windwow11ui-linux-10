...
const NotebookApp: React.FC<AppComponentProps> = ({setTitle, initialData}) => {
  ...
  useEffect(() => {
    ...
    const loadFileByPath = async (path: string) => {
      const fileData = await window.electronAPI.notebook.readFile(path);
      if (fileData) {
  ...
  const handleSave = async () => {
    if (filePath) {
      ...
      await window.electronAPI.notebook.saveFile(filePath, content);
      setIsDirty(false);
      // In a real app, we'd trigger a refresh of the file explorer here
      // triggerRefresh?.();
    } else {
